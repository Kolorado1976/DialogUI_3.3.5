...
function DQuestFrame_OnEvent(event)
    DEFAULT_CHAT_FRAME:AddMessage("DEBUG: DQuestFrame_OnEvent: " .. tostring(event));
    ...
    if (event == "QUEST_GREETING") then
        DEFAULT_CHAT_FRAME:AddMessage("DEBUG: QUEST_GREETING received in DQuestFrame");
    ...
    if (event == "QUEST_DETAIL") then
        DEFAULT_CHAT_FRAME:AddMessage("DEBUG: Received QUEST_DETAIL");
        DQuestFrameRewardPanel:Hide();
        DQuestFrameProgressPanel:Hide();
    ...
    elseif (event == "QUEST_PROGRESS") then
        DEFAULT_CHAT_FRAME:AddMessage("DEBUG: Received QUEST_PROGRESS"); 
        DQuestFrameRewardPanel:Hide();
        DQuestFrameDetailPanel:Hide();
        DQuestFrameGreetingPanel:Hide();
    ...
    elseif (event == "QUEST_COMPLETE") then
        DEFAULT_CHAT_FRAME:AddMessage("DEBUG: Received QUEST_COMPLETE");
        DQuestFrameProgressPanel:Hide();
        DQuestFrameDetailPanel:Hide();
        DQuestFrameGreetingPanel:Hide();
    ...
    elseif (event == "QUEST_ITEM_UPDATE") then
        DEFAULT_CHAT_FRAME:AddMessage("DEBUG: Received QUEST_ITEM_UPDATE");
...
function DQuestFrameGreetingPanel_OnShow()
    ...
    if gossipData then
        ...
        DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG LOADED: avail count=%d", availCount))
        for i = 1, availCount do
            DEFAULT_CHAT_FRAME:AddMessage(string.format("  [%d] = %s", i, tostring(gossipAvailableQuests[i])))
        end
        ...
        DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG: Using SAVED GOSSIP DATA - active=%d, available=%d (raw=%d/%d)", 
        ...
        if numActiveQuests == 0 and numAvailableQuests == 0 then
            ...
            DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG: Using GOSSIP API - active=%d, available=%d", 
                numActiveQuests, numAvailableQuests));
        else
            DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG: Using STANDARD API - active=%d, available=%d",
                numActiveQuests, numAvailableQuests));
        end
    end
...
function DQuestTitleButton_OnClick()
    DEFAULT_CHAT_FRAME:AddMessage("DEBUG: DQuestTitleButton_OnClick executed!");
    ...
    DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG: Button ID: %d, isActive: %s, isGossip: %s, type: %s", 
    ...
    if isGossip then
        ...
        if isActive == 1 then
            DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG: Selecting Standard Active Quest %d", buttonID));
            SelectActiveQuest(buttonID);
        else
            DEFAULT_CHAT_FRAME:AddMessage(string.format("DEBUG: Selecting Standard Available Quest %d", buttonID));
...
function DQuestFrameItems_Update(questState)
    ...
    if (numQuestChoices > 0) then
        ...
        for i = 1, numQuestChoices, 1 do
            ...
            -- ИСПРАВЛЕНО: Устанавливаем цвет текста в зависимости от качества
            local itemNameText = getglobal(questItemName .. index .. "Name");
            if itemNameText then
                itemNameText:SetText(name);
                -- Устанавливаем цвет по качеству предмета
                if quality then
                    local r, g, b = GetItemQualityColor(quality);
                    itemNameText:SetTextColor(r, g, b);
                else
                    itemNameText:SetTextColor(1.0, 1.0, 1.0); -- Белый по умолчанию
                end
            end
    ...
    if (numQuestSpellRewards > 0) then
        ...
        -- ИСПРАВЛЕНО: Цвет для заклинаний (обычно золотой/жёлтый)
        local spellNameText = getglobal(questItemName .. rewardsCount .. "Name");
        if spellNameText then
            spellNameText:SetText(name);
            spellNameText:SetTextColor(1.0, 0.82, 0.0); -- Золотой цвет для заклинаний
        end
        ...
        for i = 1, numQuestRewards, 1 do
            ...
            if rewardNameText then
                ...
                if quality then
                    ...
                    rewardNameText:SetTextColor(1.0, 1.0, 1.0); -- Белый по умолчанию
                end
            end
            ...
            if (i > 1) then
                ...
                questItem:SetPoint("TOPLEFT", questState .. "ItemReceiveText", "BOTTOMLEFT", -3, -20);
            end
            rewardsCount = rewardsCount + 1;
            lastAnchorFrame = questItem;
        end
        
        -- Блок XP (без изменений)
        if xp and xp > 0 then
            ...
            if rewardsCount > baseIndex then
                ...
                xpIcon:SetPoint("TOPLEFT", questItemName .. lastItemIndex, "BOTTOMLEFT", 0, -30);
                xpIcon:Show();
                ...
                xpIcon:ClearAllPoints();
                xpIcon:SetPoint("TOPLEFT", questItemReceiveText, "BOTTOMLEFT", 0, -25);
                xpIcon:Show();
                xpRewardText:ClearAllPoints();
                xpRewardText:SetPoint("LEFT", questState .. "XPIcon", "RIGHT", 5, 0);
            end
